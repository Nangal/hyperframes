...
**Critical clarification: this rule kills static webpage layouts, NOT designed product moments.** An animated UI demonstrating what the product does — a prompt box where text types itself + Generate fires + a video player slides up with the output, a language switcher that re-translates a subtitle live, a card sliding through a real kanban — IS legitimate cinematic content when shot well (close framing, camera motion, depth, light). The fail mode this rule addresses: a flat 1920×1080 page-layout reproduction with 2px breathing. The fail mode that OVER-correcting on this rule produces: every middle beat dissolves into abstract atmosphere because the agent refuses to compose any UI. Designed product UI IS a first-class beat type (see [step-3-storyboard.md](references/step-3-storyboard.md) — three beat content types). Frame it cinematically and animate it as a shot, don't replicate a screenshot — but DO compose it.
...
| `capture-video <dir>` | `npx tsx packages/cli/src/cli.ts capture-video <dir> --index <N>` | DIRECTORY + `--index N` or `--url <URL>` | On-demand fetch of a video listed in `capture/extracted/video-manifest.json`. Capture deliberately skips downloading mp4s (sites with 15+ feature videos would balloon the capture). Use this when a beat needs the real hero video (e.g. heygen's 3D Orb). `--list` to see entries. |
