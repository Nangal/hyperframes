...
- **Reproduce the named `effects` / `blueprint` from their recipe bodies — never approximate from the name.** Every id has a real recipe under `ANIM_DIR` (`rules/<id>.md` per effect, `blueprints/<id>.md` per blueprint); guessing from the name loses the signature (`avatar-cloud-network` guessed becomes a plain stagger, not the SVG-connected elliptical ring it actually is). Three modes — the note tells you which (`Reproduce` / `Adapt` / `Compose`): **(1) Reproduce** — a `blueprint:` is named and the note matches it → read `blueprints/<id>.md` (run the linked `examples/<id>.html` to see the signature) and **reproduce its phases faithfully**, swapping in this frame's content / assets / timing. **(2) Adapt** — a `blueprint:` is named but the note leads with a `Base / Keep / Depart` line → read the recipe + example first, then **build what the note says; the note is authoritative over the blueprint's original form.** Preserve the `Keep` signature (the move that makes the blueprint itself — the SVG ring, the orbit→collapse); apply each `Depart`; you may extend or re-seat, **never reduce below `entrance → development → settle`**. **(3) Compose** — no blueprint named → read each cited effect's `rules/<id>.md` and **sequence the ≥3 effects into the shot's phases** (one enters, one develops, one emphasizes), not all fired at once. Place each candidate by its `roles` (the `focal` is the hero): a `cutout` as a foreground subject with text laid out around it (not over its face); a `background` full-bleed, dimmed ~30–50% so foreground content stays legible. Keep imagery inside the top-83% safe area too. **A `[video]` candidate (`.mp4`) is a real motion clip — for a motion/demo product it's usually the strongest hero.** Render it as a **muted** `<video class="clip">` (`data-start` / `data-duration` / `data-track-index` per the core clip contract), a **direct child of the frame root** — never nested inside another timed element, or the renderer freezes it. The root owns all audio, so keep it muted; a `[video-still]` or untagged image → `<img>`.
...
2. **Design** — translate `scene` + the (sometimes shot-by-shot) narrative + the recipes you just read into a visual plan using `frame.md`'s components and type ramp. Plan the shot across its full `duration` — `entrance → development → settle` — honoring the note's phases shot-by-shot; never collapse a multi-phase note to one move or leave the post-entrance duration frozen. Find a visual idea that reinforces the beat, not a literal restyle of the words. Place the named assets.
...
- **Shot develops (not a slide)** — a non-still frame carries a development beat between entrance and settle; cited effects are sequenced into phases, not all fired at `t=0`. Animate-in-then-freeze under a lone macro drift is a slideshow — add the development the note specifies.
